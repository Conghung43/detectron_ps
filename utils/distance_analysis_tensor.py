...
def main(sorted_object_polygon, threadhold_config):
    ...
    for index, object_polygon in enumerate(sorted_object_polygon):
        if index >= len(sorted_object_polygon)-1:
            break
        object_polygon = torch.FloatTensor(object_polygon)
        sorted_object_polygon[index+1] = torch.FloatTensor(sorted_object_polygon[index+1])
...
def get_shortest_distance(R1,R2):
    dists = torch.cdist(R1,R2)
    return dists.min()

def find_error_position(R1, R2, threadhold_config):
    r1_min = R1[torch.where(R1[:,0] == R1[:,0].min())][0]
    r1_max = R1[torch.where(R1[:,0] == R1[:,0].max())][0]
    r2_min = R2[torch.where(R2[:,0] == R2[:,0].min())][0]
    r2_max = R2[torch.where(R2[:,0] == R2[:,0].max())][0]
    if r1_min[0] > r2_min[0]:
        head_distance = torch.cdist(torch.tensor([r1_min.tolist()]),R2).min()
        r_min = r1_min
    else:
        head_distance = torch.cdist(torch.tensor([r2_min.tolist()]),R1).min()
        r_min = r2_min
    if r1_max[0] < r2_max[0]:
        tail_distance = torch.cdist(torch.tensor([r1_max.tolist()]),R2).min()
        r_max = r1_max
    else:
        tail_distance = torch.cdist(torch.tensor([r2_max.tolist()]),R1).min()
        r_max = r2_max
    if head_distance < threadhold_config/2 and tail_distance < threadhold_config/2:
        print('None')
